	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*else if(empOrMgr == "mgr" || empOrMgr.contentEquals("mgr")) {
			...
			if(m.equals(null) || m == null) {
				...
				m.setLastName(lName);
				Manager newMgr = new Manager(m.getId(), uName, pw, fName, lName);
				new MgrService().updateMgr(newMgr);
				RequestDispatcher disp = request.getRequestDispatcher("./changeInfo.html");
			...
			if(e.equals(null) || e == null) {
				...
				//new EmpService().updateEmp(e);
				e.setUsername(uName);
				e.setPassword(pw);
				e.setFirstName(fName);
				e.setLastName(lName);
				Employee newEmp = new Employee(e.getId(), uName, pw, fName, lName, e.getmId());
				new EmpService().updateEmp(newEmp);
				RequestDispatcher disp = request.getRequestDispatcher("./changeInfo.html");
				disp.forward(request, response);
			}
		}
